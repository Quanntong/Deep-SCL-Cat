...
本项目由以下四位成员合作完成，各自负责不同的模块和任务：

### 团队成员与职责

| 成员 | 负责模块 | 主要贡献 |
|------|----------|----------|
| **曲善桐** | 项目架构设计与核心算法实现 | 1. 设计混合专家架构工作流<br>2. 实现CatBoost模型训练与优化<br>3. 开发阈值寻优策略<br>4. 编写主程序入口(main.py) |
| **蒋子怡** | 数据预处理与特征工程 | 1. 实现数据加载与清洗模块(data_loader.py)<br>2. 设计SCL-90因子语义化映射<br>3. 开发K-Means聚类分析(feature_cluster.py)<br>4. 创建数据标准化与特征增强流程 |
| **卢勤** | 模型评估与可解释性分析 | 1. 实现多模型对比分析(compare_models.py)<br>2. 开发SHAP可解释性模块(explainability.py)<br>3. 设计可视化图表生成<br>4. 编写模型性能评估报告 |
| **马忠义** | Web界面开发与系统集成 | 1. 开发Streamlit Web界面(app.py)<br>2. 实现单体预测与批量筛查功能<br>3. 设计用户友好的交互界面<br>4. 集成模型预测与结果展示 |

### 协作流程

1. **需求分析阶段**：全体成员共同讨论项目目标与业务需求
2. **架构设计阶段**：曲善桐主导，其他成员提供模块化建议
3. **并行开发阶段**：各成员按分工独立开发负责模块
4. **集成测试阶段**：马忠义负责Web界面集成，全体成员参与测试
5. **文档撰写阶段**：各成员撰写对应模块文档，曲善桐统一整合
